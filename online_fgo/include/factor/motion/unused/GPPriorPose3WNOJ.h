...
#ifndef ONLINE_FGO_GPPRIORPOSE3WNOJ_H
#define ONLINE_FGO_GPPRIORPOSE3WNOJ_H
...
namespace fgo {
  namespace factors {
    ...
    class GaussianProcessPriorPose3WNOJ : public gtsam::NoiseModelFactor4<gtsam::Pose3,
            gtsam::Vector3, gtsam::Pose3, gtsam::Vector3> {

    private:
      typedef GaussianProcessPriorPose3WNOJ This;
      ...
      gtsam::Vector3 acca1_;
      gtsam::Vector3 accg1_;
      gtsam::Vector3 acca2_;
      gtsam::Vector3 accg2_;

    public:
      GaussianProcessPriorPose3WNOJ() {}    /* Default constructor only for serialization */

      /// Constructor
      /// @param delta_t is the time between the two states
      GaussianProcessPriorPose3WNOJ(
              gtsam::Key poseKey1, gtsam::Key velKey1, gtsam::Key poseKey2, gtsam::Key velKey2,
              const gtsam::Vector3& omega1, const gtsam::Vector3& omega2, const gtsam::Vector3& acca1,
              const gtsam::Vector3& accg1, const gtsam::Vector3& acca2, const gtsam::Vector3& accg2,
              ...
              Base(gtsam::noiseModel::Gaussian::Covariance(fgo::utils::calcQ<6>(fgo::utils::getQc(Qc_model), delta_t).block<9,9>(0,0)),
                   poseKey1, velKey1, poseKey2, velKey2), omega1_(omega1), omega2_(omega2),
              acca1_(acca1), accg1_(accg1), acca2_(acca2), accg2_(accg2) {
        delta_t_ = delta_t;
      }

      virtual ~GaussianProcessPriorPose3WNOJ() {}
      ...
      gtsam::Vector evaluateError_(const gtsam::Pose3 &pose1, const gtsam::Vector3 &vel1_n,
        ...
        gtsam::Vector6 acc1 = fgo::utils::convertGbAbtoGbAw(accg1_,acca1_,pose1);
        gtsam::Vector6 acc2 = fgo::utils::convertGbAbtoGbAw(accg2_,acca2_,pose2);

        gtsam::Vector6 r = gtsam::Pose3::Logmap(gtsam::Pose3(pose1.rotation().inverse() * pose2.rotation(),
                                                             - pose1.translation() + pose2.translation()));
        return  (gtsam::Vector9() << (r - vel1 * delta_t_ - acc1/2 * pow(delta_t_,2)),
                (pose1.rotation().transpose() * vel2.block<3,1>(3,0) -
                        pose1.rotation().transpose() * (vel1.block<3,1>(3,0) + acca1_*delta_t_))).finished();
...
namespace gtsam {
  template<>
  struct traits<fgo::factors::GaussianProcessPriorPose3WNOJ> : public Testable<fgo::factors::GaussianProcessPriorPose3WNOJ> {};
}



#endif //ONLINE_FGO_GPPRIORPOSE3WNOJ_H
