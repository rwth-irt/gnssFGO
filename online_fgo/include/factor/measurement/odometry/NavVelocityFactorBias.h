...
class NavVelocityFactor : public gtsam::NoiseModelFactor3<gtsam::Pose3, gtsam::Vector3, gtsam::imuBias::ConstantBias>
{
protected:
...
public:
    ...
    NavVelocityFactor(gtsam::Key poseKey, gtsam::Key velKey, gtsam::Key biasKey, const gtsam::Vector3 &velMeasured, const gtsam::Vector3& angularVelocity,
        ...
        Base(model, poseKey, velKey, biasKey), velMeasured_(velMeasured), angularVelocity_(angularVelocity), lb_(lb), measuredVelFrame_(velFrame), type_(type), useAutoDiff_(useAutoDiff)
        {}
    ...
    /** factor error */
    [[nodiscard]] gtsam::Vector evaluateError(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::imuBias::ConstantBias& bias,
      ...
      if(useAutoDiff_) {
          ...
          switch(type_)
          {
            case VelocityType::VEL3D: {
              if (H1) {
                *H1 = gtsam::numericalDerivative11<gtsam::Vector3, gtsam::Pose3>(
                    boost::bind(&This::evaluateError_, this, boost::placeholders::_1, vel, bias), pose, 1e-5);

              }
              if (H2) {
                *H2 = gtsam::numericalDerivative11<gtsam::Vector3, gtsam::Vector3>(
                    boost::bind(&This::evaluateError_, this, pose, boost::placeholders::_1, bias), vel, 1e-5);
              }
              if (H3) {
                *H3 = gtsam::numericalDerivative11<gtsam::Vector3, gtsam::imuBias::ConstantBias>(
                    boost::bind(&This::evaluateError_, this, pose, vel, boost::placeholders::_1), bias, 1e-5);
            ...
            case VelocityType::VEL2D: {
              ...
              if (H3) {
                *H3 = gtsam::numericalDerivative11<gtsam::Vector2, gtsam::imuBias::ConstantBias>(
                    boost::bind(&This::evaluateError_, this, pose, vel, boost::placeholders::_1), bias, 1e-5);
            ...
            case VelocityType::VELY: {
              ...
              if (H3) {
                *H3 = gtsam::numericalDerivative11<gtsam::Vector1, gtsam::imuBias::ConstantBias>(
                    boost::bind(&This::evaluateError_, this, pose, vel, boost::placeholders::_1), bias, 1e-5);
              }
              break;
            }
      ...
      {
        gtsam::Matrix Hpos, Hrot, Hrot1, Hbias;
        const auto& pos = pose.translation(Hpos);
        const auto& rot = pose.rotation(Hrot1);
        const auto lbv_b = gtsam::skewSymmetric(-lb_) * bias.correctGyroscope(angularVelocity_, Hbias);
        gtsam::Vector3 error;
        ...
        switch (measuredVelFrame_) {
          ...
          case MeasurementFrame::BODY:
          {
            ...
            if(H2) HH2 = Hrot_vel;  // should be NOT COMPLETELY CORRECT
            if(H3) HH3 = gtsam::skewSymmetric(lb_) * Hbias;
            break;
          }
          default: {
            const auto lbv = rot.rotate(lbv_b);
        ...
        switch (type_) {
          ...
          case VEL2D: {
            ...
            if(H3) *H3 = HH3.block<2, 6>(0, 0);
            return error.head(2);
          }
          case VELX: {
            if(H1) *H1 = HH1.block<1, 6>(0, 0);
            if(H2) *H2 = HH2.block<1, 3>(0, 0);
            if(H3) *H3 = HH3.block<1, 6>(0, 0);
    ...
    [[nodiscard]] gtsam::Vector evaluateError_(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::imuBias::ConstantBias& bias) const {

      gtsam::Vector3 error;
      auto lbv_b = gtsam::skewSymmetric(-lb_) * bias.correctGyroscope(angularVelocity_);
