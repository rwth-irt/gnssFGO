...
namespace fgo::factor {
    class PrDrFactor : public gtsam::NoiseModelFactor4<gtsam::Pose3, gtsam::Vector3, gtsam::imuBias::ConstantBias, gtsam::Vector2> {
    ...
    public:
        ...
        PrDrFactor(gtsam::Key pose_i, gtsam::Key vel_i, gtsam::Key bias_i, gtsam::Key cbd_i,
                   ...
                   const gtsam::Vector3 &satVEL, const gtsam::Vector3 &lb, const gtsam::Vector3 &omega, const gtsam::SharedNoiseModel &model, bool useAutoDiff = false) :
            ...
            {
              factorTypeID_ = FactorTypeIDs::PRDR;
        ...
        gtsam::Vector evaluateError(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::imuBias::ConstantBias &bias1,
          ...
          } else
          {
            ...
            gtsam::Point3 P_eA_e = pose.translation(&Hpose_P) + pose.rotation(&Hpose_Pr).rotate(lb_, &Hrho_p); //pose.compose(body_P_sensor,H0_P).translation(Hpose_P);
            ...
            if (H3) *H3 = (gtsam::Matrix26() << 0, 0, 0, 0, 0, 0, Hd * Hdrho_t * gtsam::skewSymmetric(lb_) * Hbias_gyro).finished();
        ...
        [[nodiscard]] gtsam::Vector evaluateError_(const gtsam::Pose3 &pose, const gtsam::Vector3 &vel, const gtsam::imuBias::ConstantBias &bias1,
